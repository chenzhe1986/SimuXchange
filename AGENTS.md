...
- **上交所竞价协议编码**：报文头 MsgType(4)+MsgSeqNum(8)+MsgBodyLen(4)（共 16 字节）+Body+Checksum(4)，
  ...
  **登录后需先完成分区序号同步(206→207)才推送执行报告**（同步前回报缓存，同步后补发）；
  新订单 58 → 申报响应 32 / 成交 103 / 拒单 204；撤单 61 固定回撤单失败 59；心跳 33，区间[5,60]。
- **上交所新债券协议编码**：报文结构与竞价平台完全一致（报文头、校验和、字段放大、会话流程、消息类型均同竞价），
  仅业务参数不同：PlatformID=2（竞价=0）、SetID 默认 801（竞价=1）、BizID=1 债券现券竞价 / 2 债券质押式回购（竞价=100010）、
  协议版本最低 1.90（竞价用 "0.50"）、OrdType 仅支持 2=限价。
- **七种回报策略**（三套协议一致）：全成单笔 / 全成拆单 / 部成单笔 / 部成拆单 / 自定义成交 /
  只回确认 / 拒单（执行报告拒绝或业务拒绝二选一）。
...
- 修改协议/策略逻辑后必须跑 `cargo test -p simx-core`（含 11 个 e2e 测试：深交所 3 + 上交所竞价 4 + 上交所新债券 4）；
- 右键菜单、弹窗等 UI 交互遵循现有实现风格（贴边修正、运行态置灰、Esc 关闭）；
- 端口约定：Vite 开发端口 **14200**（`vite.config.ts` 与 `tauri.conf.json` 的
  devUrl 两处必须一致）；e2e 测试占用 18101~18103（深交所）、18201~18204（上交所竞价）与 18301~18304（上交所新债券）；平台默认端口从 10001 起。
...
- [ ] 涉及协议/策略/会话时：`cargo test -p simx-core` 全绿（64 单元 + 4 自动启动 + 11 e2e）
