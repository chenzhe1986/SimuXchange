//! 上交所新债券平台端到端集成测试：引擎启动 → 柜台 TCP 连接 → Logon →
//! 分区序号同步 → 委托 → 校验回报。
//!
...
//! 各测试使用独立端口（18301/18302/18303/18304），互不干扰。

use simx_core::config::{
    GatewayCategory, GatewayConfig, PlatformConfig, StrategyConfig, StrategyMode,
};
use simx_core::engine::Engine;
use simx_core::shbond::protocol::{self as protocol, msg_type, BodyWriter, Logon, BIZ_ID_CASH_BOND};
...
fn new_order_frame(cl_ord_id: &str, qty_1000x: i64, price_n13_5: i64, side: u8) -> Vec<u8> {
    ...
    w.u32(BIZ_ID_CASH_BOND); // BizID
    w.str("PBU00001", 8); // BizPbu
    w.str(cl_ord_id, 10); // ClOrdID
    w.str("019547", 12); // SecurityID
...
async fn connect_logon_sync(port: u16) -> TcpStream {
    ...
    stream.write_all(&sync_frame("OMS_TEST", 801)).await.unwrap();
    let (mt, _) = read_frame(&mut stream).await;
    assert_eq!(mt, msg_type::EXEC_RPT_SYNC_RSP, "同步后应收到同步响应(207)");
    stream
}

/// 拼一个最小可用的上交所新债券测试网关配置
fn test_gateway(port: u16, strategy: StrategyConfig) -> GatewayConfig {
    GatewayConfig {
        id: String::new(),
        name: "上海新债券测试网关".into(),
        category: GatewayCategory::Shbond,
        platforms: vec![PlatformConfig {
            id: String::new(),
            name: "新债券平台".into(),
            platform_type: 2, // 新债券平台 PlatformID=2
            partition_no: 801, // 分区号 SetID=801
...
#[tokio::test]
async fn test_shbond_full_single_sync_flow() {
    let dir = std::env::temp_dir().join(format!("simx_shbond_{}", std::process::id()));
    ...
    let mut stream = connect_logon_sync(18301).await;

    // 买入 019547，500 股 @ 12.34（500_000 = 500 股乘 1000；12_34000 = 12.34 元乘 10 万）
    stream
...
#[tokio::test]
async fn test_shbond_sync_gate_buffers_reports() {
    let dir = std::env::temp_dir().join(format!("simx_shbond_gate_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let engine = Engine::new(dir.clone());
    ...
    // 仅登录、不做序号同步，直接发委托
    let mut stream = connect_and_logon(18302).await;
    stream
...
#[tokio::test]
async fn test_shbond_cancel_reject_and_logout() {
    let dir = std::env::temp_dir().join(format!("simx_shbond_cxl_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let engine = Engine::new(dir.clone());

    let gw = engine
        .save_gateway(test_gateway(18303, StrategyConfig::default()))
        .await
        .unwrap();
    engine.start_gateway(&gw.id).await.unwrap();

    let mut stream = connect_logon_sync(18303).await;
...
#[tokio::test]
async fn test_shbond_split_trades_async_delay() {
    let dir = std::env::temp_dir().join(format!("simx_shbond_split_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let engine = Engine::new(dir.clone());
    ...
    let gw = engine.save_gateway(test_gateway(18304, strategy)).await.unwrap();
    engine.start_gateway(&gw.id).await.unwrap();

    let mut stream = connect_logon_sync(18304).await;
    stream
...
#[tokio::test]
async fn test_shbond_sync_resends_history_by_begin() {
    let dir = std::env::temp_dir().join(format!("simx_shbond_rsync_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let engine = Engine::new(dir.clone());
    let gw = engine
        .save_gateway(test_gateway(
    ...
    let mut s1 = connect_logon_sync(18305).await;
    ...
    let mut s2 = connect_logon_sync(18305).await;
